...
| Headers                           | Em `next.config.ts` pra todas as rotas: CSP com `default-src 'self'`, `script-src 'self' 'unsafe-inline'` (+ hosts da Cloudflare; sem nonce porque App Router e next-themes usam script inline), `img-src 'self' data: blob: https://tile.openstreetmap.org`, `connect-src 'self'`, `worker-src 'self' blob:`, `frame-ancestors 'none'`, `form-action 'self'`, `object-src 'none'`; mais `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: geolocation=(self), camera=(self), microphone=()` |
