...
| Headers                           | CSP (`default-src 'self'`; `img-src 'self' data: blob: https://tile.openstreetmap.org`; `connect-src 'self'`), `X-Content-Type-Options: nosniff`, `Referrer-Policy: same-origin`, `Permissions-Policy: geolocation=(self)`                                                                                                       |
