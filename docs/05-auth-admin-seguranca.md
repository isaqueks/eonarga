...
| Upload malicioso (v2)             | Aceita só `image/jpeg, png, webp, heic`; confere magic bytes; máx. 10 MB; **reprocessa** com sharp (gera webp novo, o que destrói payload e remove EXIF/GPS); nome de arquivo = id gerado. Vídeo (60 MB) e áudio (20 MB) entram como vieram, conferidos pelo contêiner nos magic bytes, servidos só com sessão e com `nosniff`; duração e forma de onda vindas do navegador são validadas (inteiro no teto, lista de 0..1) e servem só pra exibição                                                                                                                                                                                                                                   |
