...
const SELF = "Se cutucar sozinho não vale.";
const TOO_SOON = "Calma. Uma cutucada por minuto.";

/** Uma cutucada por minuto por pessoa, seja lá quem for o alvo (docs/08 #45). */
const POKE_LIMIT = { limit: 1, windowMs: 60_000 };

/** Vibra, pausa, vibra, pausa, vibra (ms): a cutucada tem que ser sentida no bolso. */
const POKE_VIBRATION = [300, 100, 300, 100, 500];

/**
 * "Cutucar" na galera: um push só pra pessoa escolhida, "Fulano cutucou você", que abre
 * a galera e vibra o celular. Sem confirmação (é uma pessoa só) e com trava de uma por
 * minuto no servidor, porque cutucada em série é o mesmo incômodo que chamada em série.
 */
export async function pokeUser(targetId: string): Promise<CallGroupState> {
  ...
  // Só conta a tentativa depois de validar: cutucar quem não existe não gasta a vez.
  ...
  const payload: PushPayload = {
    title: "E o narga?",
    body: `${user.name} cutucou você`,
    url: "/galera",
    // Várias cutucadas da mesma pessoa trocam o balão em vez de empilhar.
    tag: `poke:${user.id}`,
