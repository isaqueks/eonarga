...
export interface PokeRowProps {
  userId: string;
  /** Nome de quem vai ser cutucado, pro rótulo acessível e pro aviso de "sem push". */
  name: string;
...
/**
 * Última linha do card da galera: o "visto há…" à esquerda e "Cutucar" à direita, com o
...
export function PokeRow({ userId, name, enabled, children }: PokeRowProps) {
  ...
  function poke() {
    ...
    startTransition(async () => {
      ...
      if (!state.ok) {
        setResult({ tone: "error", message: state.error ?? "Não rolou cutucar. Tenta de novo." });
        return;
      }
      setResult(
        (state.recipients ?? 0) > 0
          ? { tone: "ok", message: "Cutucou!" }
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          ...
          disabled={pending}
          onClick={poke}
          aria-label={`Cutucar ${name}`}
        >
          <Pointer className="size-3.5" aria-hidden />
          {pending ? "Cutucando…" : "Cutucar"}
        </Button>
      </div>
      {result ? (
        <p
